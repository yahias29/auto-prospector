...
@app.get("/")
def read_root():
    return {"status": "API is running"}  # keep exactly this for tests

@app.post("/process_lead", response_model=LeadOutput)
async def process_lead(lead: LeadInput):
    # Lazy import here to avoid startup-time imports
    ...
    output_data = LeadOutput(
        **lead.model_dump(),
        ...
        personalized_message=getattr(ai_result["personalized_message"], "raw", ai_result["personalized_message"]),
        is_new_lead=True
    )
